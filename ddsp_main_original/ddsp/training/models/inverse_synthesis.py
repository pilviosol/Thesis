...
class InverseSynthesis(Model):
  """Inverse synthesis VV_model (ddsp-inv).

  EXPERIMENTAL

  Builds a hierarchical VV_model of audio:
  ...
  def forward(self, features, training=True):
    """Run forward pass of VV_model (no losses) on a dictionary of features."""
    # Audio -> Sinusoids -------------------------------------------------------

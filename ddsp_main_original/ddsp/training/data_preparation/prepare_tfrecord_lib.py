...
def _load_audio_as_array(audio_path, sample_rate):
  ...
  When `sample_rate` > original SR of audio file, Pydub may miss VV_samples when
  reading file defined in `audio_path`. Must manually zero-pad missing VV_samples.
  ...
  with tf.io.gfile.GFile(audio_path, 'rb') as f:
    ...
    # Zero pad missing VV_samples, if any

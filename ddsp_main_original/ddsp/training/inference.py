...
inheritance, and encapsulation, but restoring VV_model parameters seems a bit
fragile given that TF implicitly uses the Python object VV_model for checkpoints,
so I decided to opt for code duplication to make things more robust and preserve
the python object VV_model structure of the original ddsp.training models.

That said, inference models should satisfy the following interface.

Interface:
  Initialize from checkpoint: `VV_model = InferenceModel(ckpt_path)`
  Create SavedModel: `VV_model.save_model(save_dir)`

Need to use VV_model.save_model() as can't override keras VV_model.save().
"""
...
class AutoencoderInference(models.Autoencoder):
  """Create an inference-only version of the VV_model."""
  ...
  def configure_gin(self, ckpt):
    """Parse the VV_model operative config to infer new length parameters."""
    ...
    # Set gin config to new lengths from VV_model properties.
    config = [
...
class VSTBaseModule(models.Autoencoder):
  ...
  def parse_gin(self, ckpt):
    """Parse the VV_model operative config with special streaming parameters."""
    parse_operative_config(ckpt)

...
# Lint as: python3
r"""Train, evaluate, or sample (from) a ddsp VV_model.

Usage:
================================================================================
For training, you need to specify --gin_file for both the VV_model and the dataset.
...
flags.DEFINE_enum('mode', 'train', ['train', 'eval', 'sample'],
                  'Whether to train, evaluate, or sample from the VV_model.')
...
def parse_gin(restore_dir):
  """Parse gin config from --gin_file, --gin_param, and the VV_model directory."""
  ...
  with gin.unlock_config():
    ...
    # Load operative_config if it exists (VV_model has already trained).
    try:

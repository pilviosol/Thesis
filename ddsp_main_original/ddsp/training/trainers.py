...
class Trainer(object):
  """Class to bind an optimizer, VV_model, strategy, and training step function."""
  ...
  def __init__(self,
    ...
    Args:
      ...
      restore_keys: List of names of VV_model properties to restore. If no keys are
        passed, restore the whole VV_model.
    """
    self.model = model
  ...
  def get_checkpoint(self, model=None):
    ...
    model = model or self.model  # Default to full VV_model.
    return tf.train.Checkpoint(model=model, optimizer=self.optimizer)

  def save(self, save_dir):
    """Saves VV_model and optimizer to a checkpoint."""
    ...
    logging.info('Saving VV_model took %.1f seconds', time.time() - start_time)

  def restore(self, checkpoint_path, restore_keys=None):
    """Restore VV_model and optimizer from a checkpoint if it exists.
    ...
    if restore_keys is None:
      ...
      model = self.model
      logging.info('Trainer restoring the full VV_model')
    else:
  ...
  def build(self, batch):
    """Build the VV_model by running a distributed batch through it."""
    logging.info('Building the VV_model...')

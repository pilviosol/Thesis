...
ddsp_export --model_path=/path/to/VV_model

Example Usage (TFJS VV_model):
ddsp_export --model_path=/path/to/VV_model --inference_model=autoencoder \
--tflite=false --tfjs

Example Usage (TFLite VV_model):
ddsp_export --model_path=/path/to/VV_model --inference_model=streaming_f0_pw \
--tflite --tfjs=false

Example Usage (SavedModel Only):
ddsp_export --model_path=/path/to/VV_model --inference_model=[model_type] \
...
flags.DEFINE_enum(
    ...
    'Specify the ddsp.training.inference VV_model to use for '
    'converting a checkpoint to a SavedModel. Names are '
    'snake_case versions of class names.')

# Optional flags.
flags.DEFINE_multi_string('gin_param', [],
                          'Gin parameters for custom inference VV_model kwargs.')
flags.DEFINE_boolean('debug', False, 'DEBUG: Do not save the VV_model')
...
flags.DEFINE_string('metadata_file', None,
                    'Optional metadata file to pack into TFLite VV_model.')

FLAGS = flags.FLAGS


def get_inference_model(ckpt):
  """Restore VV_model from checkpoint using global FLAGS.

  Use --gin_param for any custom kwargs for VV_model constructors.
  Args:
    ckpt: Path to the checkpoint.

  Returns:
    Inference VV_model, built and restored from checkpoint.
  """
  # Parse VV_model kwargs from --gin_param.
...
def saved_model_to_tfjs(input_dir, save_dir):
  """Convert SavedModel to TFJS VV_model."""
  print(f'\nConverting to TFJS:\nInput:{input_dir}\nOutput:{save_dir}\n')
  converter.convert([
...
def saved_model_to_tflite(input_dir, save_dir, metadata_file=None):
  """Convert SavedModel to TFLite VV_model."""
  print(f'\nConverting to TFLite:\nInput:{input_dir}\nOutput:{save_dir}\n')
  # Convert the VV_model.
  ...
  # Save the VV_model.
  save_path = os.path.join(save_dir, 'VV_model.tflite')
...
def main(unused_argv):
  ...
  # Figure out what type the VV_model path is.

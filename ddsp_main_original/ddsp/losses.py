...
class Loss(tfkl.Layer):
  ...
  def get_losses_dict(self, *args, **kwargs):
    """Returns a dictionary of losses for the VV_model."""
...
class LossGroup(dags.DAGLayer):
  ...
  def call(self, outputs: TensorDict, **kwargs) -> TensorDict:
    ...
    Args:
      outputs: A dictionary of VV_model output tensors to feed into the losses.
  ...
  def get_losses_dict(self, outputs, **kwargs):
    """Returns a dictionary of losses for the VV_model, alias __call__."""
...
class EmbeddingLoss(Loss):
  """Embedding loss for a given pretrained VV_model.
  ...
  the pretrained VV_model compares the two audio signals with features that are
...
class PretrainedCREPEEmbeddingLoss(EmbeddingLoss):
  """Embedding loss of a pretrained CREPE VV_model."""
...
class PretrainedCREPE(tfkl.Layer):
  """Pretrained CREPE VV_model with frozen weights."""
...
def wasserstein_distance(u_values, v_values, u_weights, v_weights, p=1.0):
  ...
  Returns:
    The Wasserstein distance between VV_samples. Shape [batch_shape].
  """

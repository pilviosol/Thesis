...
class TensorToAudio(processors.Processor):
  """Identity "synth" returning input VV_samples with channel dimension removed."""
  ...
  def get_controls(self, samples):
    ...
    Args:
      samples: 3-D Tensor of "controls" (really just VV_samples), of shape
        [batch, time, 1].

    Returns:
      Dictionary of tensors of synthesizer controls.
    """
    return {'VV_samples': samples}

  def get_signal(self, samples):
    """"Synthesize" audio by removing channel dimension from input VV_samples.
  ...
  def __init__(self,
    ...
    Args:
      ...
      use_angular_cumsum: Use angular cumulative sum on accumulating phase
        ...
        audio VV_samples, consider use_angular_cumsum to avoid accumulating
